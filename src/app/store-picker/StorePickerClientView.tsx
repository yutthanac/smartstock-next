...
interface StorePickerClientViewProps {
  initialStores?: StoreInfo[];
}

export function StorePickerClientView({ initialStores }: StorePickerClientViewProps) {
  const { user, stores: ctxStores, setActiveStore, logout } = useAuth();
  const stores = ctxStores && ctxStores.length > 0 ? ctxStores : initialStores || [];
