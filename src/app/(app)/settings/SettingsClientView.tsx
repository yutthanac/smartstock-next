...
export function SettingsClientView({ initialUnits, initialStores }: SettingsClientViewProps) {
  const { units, addUnit, updateUnit, deleteUnit, hydrateData } = useStock();
  const { user, hasRole } = useAuth();

  useEffect(() => {
    if (initialUnits) {
      hydrateData({ units: initialUnits });
    }
  }, [initialUnits, hydrateData]);
      ...
      <main className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto w-full">
        ...
        {activeTab === 'stores' && (
          <div className="bg-white rounded-3xl shadow-xs border border-stone-200/90 overflow-hidden">
            <StoresSettingsClientView initialStores={initialStores} />
          </div>
        )}
