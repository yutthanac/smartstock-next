...
import { WasteStatsResponse, DashboardKPI, Order } from '@/types';
...
interface SalesReportClientViewProps {
  ...
  initialOrders?: Order[];
  initialWasteStats?: WasteStatsResponse | null;
}
...
export function SalesReportClientView({
  ...
  const { dashboard: ctxDashboard, orders: ctxOrders, fetchWasteStats, fetchRefundStats, hydrateData } = useStock();
  const [period, setPeriod] = useState<PeriodFilter>('7days');
  const [aiAnalyzing, setAiAnalyzing] = useState<boolean>(false);
  const [executiveSummary, setExecutiveSummary] = useState<string | null>(null);
  const [wasteStats, setWasteStats] = useState<WasteStatsResponse | null>(initialWasteStats || null);

  useEffect(() => {
    if (initialDashboard || initialOrders) {
      hydrateData({
        dashboard: initialDashboard || undefined,
        orders: initialOrders || undefined,
      });
    }
  }, [initialDashboard, initialOrders, hydrateData]);

  const dashboard = ctxDashboard || initialDashboard;
  const orders = ctxOrders && ctxOrders.length > 0 ? ctxOrders : initialOrders || [];
